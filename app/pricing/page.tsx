...
const tiers = [
  ...
  },
  {
    ...
    monthly: "$4.99",
    annual: "$3.33",
    tagline: "billed annually at $39.99",
    cta: "Start Pro Free Trial",
    popular: true,
    features: [
      "Everything in Free",
      "Unlimited trips & group size",
      "AI Budget Oracle",
      "Outfit planner",
      "Events & concerts discovery",
      "Group wallet & split payments",
      "Offline mode",
      "Achievements & LVR points",
      "Priority support",
      "Trip memory book",
    ],
  },
    ...
    <div className="pt-12 pb-20 max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {tiers.map((t) => (
          ...
          >
            <Card className="p-8 h-full relative" color={t.popular ? "#A78BFA" : "#7C3AED"}>
              {t.popular && (
              ...
              <div className="text-4xl font-bold font-spaceGrotesk mt-2">{annual ? t.annual : t.monthly}</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-muted text-xs uppercase tracking-widest">
                ...
                <th className="py-3 px-4 gradient-text">Pro</th>
            ...
            return (
              <div key={f.q} className="bg-card border border-border rounded-xl overflow-hidden">
                <button onClick={() => setOpenFaq(open ? null : i)} className="w-full text-left flex items-center justify-between p-5">
                  ...
                  <motion.span animate={{ rotate: open ? 45 : 0 }} className="text-purpleLight text-2xl">+</motion.span>
                </button>
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-16 text-sm text-muted">
        {["No credit card required", "Cancel anytime", "GDPR compliant", "256-bit SSL"].map((t) => (
          <div key={t} className="flex items-center gap-2 bg-card border border-border rounded-full px-4 py-2">
