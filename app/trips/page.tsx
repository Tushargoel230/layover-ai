...
const timeIcons: Record<string, string> = { morning: "☀️", afternoon: "🌤️", evening: "🌙" };

export default function TripsPage() {
  const [openDays, setOpenDays] = useState<Record<string, number[]>>({
    croatia: [1], paris: [1], netherlands: [1],
  });
      ...
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #FEF3C7, #FFEDD5, #FFF8EC)" }}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16 text-center relative">
          ...
          <div className="flex flex-wrap justify-center gap-3 text-sm text-muted">
            <span className="bg-white border border-border rounded-full px-3 py-1 shadow-sm">🇭🇷 Croatia</span>
            <span className="bg-white border border-border rounded-full px-3 py-1 shadow-sm">🇫🇷 Paris</span>
            <span className="bg-white border border-border rounded-full px-3 py-1 shadow-sm">🇳🇱 Netherlands</span>
          </div>
        </div>
      </div>
