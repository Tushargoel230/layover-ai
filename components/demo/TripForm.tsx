  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6">
      <h1 className="text-4xl md:text-5xl font-bold font-spaceGrotesk text-center mb-3">
        Plan Your Trip <span className="gradient-text-teal">with AI</span>
      </h1>
      ...
      <form onSubmit={(e)=>{e.preventDefault();onGenerate();}} className="space-y-6 bg-white border border-border rounded-2xl p-6 md:p-8 shadow-sm">
                ...
                <motion.div initial={{ opacity:0, y:-6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-6 }}
                  ...
                  {cities.map(c=>(
                    <button key={c} type="button" onMouseDown={()=>{setDestination(c);setShowCities(false);}}
                      className="px-3 py-1.5 rounded-full bg-bg border border-border text-sm hover:border-primary hover:text-primary hover:bg-primary/5 transition-colors">
                      {c}
                    </button>
                  ))}
                </motion.div>
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <div className="text-sm text-primary font-bold">${budgetValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</div>
          </div>
          <input type="range" min={500} max={10000} step={100} value={budgetValue}
            onChange={e=>setBudgetValue(parseInt(e.target.value))}
            style={{ background: `linear-gradient(to right, #0A7075 0%, #14B8C4 ${((budgetValue-500)/9500)*100}%, #CBE8EC ${((budgetValue-500)/9500)*100}%)` }} />
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <button type="button" onClick={()=>setTravelers(n=>Math.max(1,n-1))}
              className="w-10 h-10 rounded-full bg-bg border border-border hover:border-primary hover:text-primary transition-colors text-xl font-bold">−</button>
          ...
          <div className="flex gap-2 flex-wrap">
            {styles.map(s=>(
              <button key={s} type="button" onClick={()=>setStyle(s)}
                className={`px-4 py-2 rounded-full border text-sm transition-all ${style===s ? "bg-primary text-white border-primary shadow-sm" : "bg-bg border-border text-muted hover:text-text hover:border-primary"}`}>
