...
const activity = [
  { text: "Earned Budget Hero badge", time: "2 days ago" },
  { text: "Completed trip to Amsterdam", time: "1 week ago" },
  { text: "Invited 3 friends to Rome trip", time: "1 week ago" },
  { text: "Added concert to Rome itinerary", time: "2 weeks ago" },
  { text: "Split $560 hotel bill with 3 friends", time: "2 weeks ago" },
];
          ...
          <ul className="divide-y divide-border">
            {activity.map((a, i) => (
              <motion.li key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex-1 text-sm text-text">{a.text}</div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white border border-border rounded-full px-5 py-3 z-50 text-sm shadow-lg">
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
