        ...
        <svg width={220} height={220} viewBox="0 0 220 220">
          <g transform="rotate(-90 110 110)">
            {budget.map((b) => {
              ...
              const el = (
                <motion.circle key={b.name} cx={110} cy={110} r={70} fill="transparent"
          ...
          <text x="110" y="104" textAnchor="middle" fill="#64748B" style={{ fontSize: 13 }}>Total</text>
          <text x="110" y="128" textAnchor="middle" fill="#0C2340" fontWeight="bold" style={{ fontSize: 22 }}>${total}</text>
        </svg>
      </div>
      <div className="flex-1 w-full max-w-md space-y-3">
        {budget.map((b, i) => (
          <motion.div key={b.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
            ...
            <div className="flex-1">
              ...
              <div className="h-1.5 bg-border rounded-full mt-1 overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: `${(b.amount / total) * 100}%` }}
